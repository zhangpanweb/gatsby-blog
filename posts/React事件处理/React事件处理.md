...
- 不能通过返回 false 来阻止事件的默认行为，只能通过显性调用 preventDefault 进行
...
- 第一种：使用 es6 class 语法时，如果将类的方法作为事件处理函数，则需要进行 this 绑定。原因：在`onClick={this.handleClick}`中 this.handle 引用正确，于是会变成`onClick={function(){this.setState({ isRed: !this.state.isRed })}}`，所以会出现在这个方法中 this 为 undefined，于是报错。
...
class App extends Component {
  ...
  render() {
    return (
      <div
        className="welcome"
        style={{ background: this.state.isRed ? "red" : "green" }}
      >
...
class App extends Component {
  ...
  render() {
    return (
      ...
      >
        Welcome to this websit.
        <button onClick={this.handleClick.bind(this, this.id)}>
          change color
        </button>
      </div>
    );
...
class App extends Component {
  ...
  render() {
    return (
      ...
      >
        Welcome to this websit.
        <button onClick={e => this.handleClick(this.id, e)}>
          change color
        </button>
      </div>
    );
